...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
    ...
    topLeft     = float3(-size * 0.5, size * 0.5, 0);
    topRight    = float3(size * 0.5, size * 0.5, 0);
    bottomLeft  = float3(-size * 0.5, -size * 0.5, 0);
    bottomRight = float3(size * 0.5, -size * 0.5, 0);
...
float4 MainPS(GS_DATA input) : SV_TARGET
{
    ...
    float4 result = gParticleTexture.Sample(gSS_PointWrap, input.TexCoord);
...
technique10 Default
{
	pass p0 
    {
        ...
        SetDepthStencilState(gDSS_DepthNoWrite, 0);

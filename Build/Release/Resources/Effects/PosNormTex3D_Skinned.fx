...
VS_OUTPUT VS(VS_INPUT input){
	...
	for(int i{};i<4;++i)
	{
		...
		if(index > -1)
		{
			transformedPosition +=mul( mul(originalPosition,input.blendWeights[i]), gBones[index]);
			transformedNormal +=(mul( mul(input.normal,input.blendWeights[i]), (float3x3)gBones[index]));

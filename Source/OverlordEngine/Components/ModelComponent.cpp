...
{
    gameContext.pShadowMapRenderer->GetMaterial(!m_pMeshFilter->m_AnimationClips.empty())->SetEffectVariables(gameContext, this);
    gameContext.pShadowMapRenderer->Draw(gameContext, m_pMeshFilter);
};
...
{ 
    ...
    auto shadowMap = m_pMaterial->m_pEffect->GetVariableByName("gShadowMap");
	if(shadowMap->IsValid())
	{
		shadowMap->AsShaderResource()->SetResource(0);
        m_pMaterial->GetTechnique()->GetPassByIndex(0)->Apply(0, gameContext.pDeviceContext);
	}
};

...
void SoftwareSkinningScene_2::Update(const GameContext& gameContext)
{
	...
	for (int i = 0; i < 24; ++i)
	{
		...
		XMVECTOR vec = XMVector3Transform(XMLoadFloat3(&pos), boneTransform0);
		XMStoreFloat3(&m_SkinnedVertices[i].TransformedVertex.Position, vec);
	}
	for (int i = 24; i < 48; ++i)
	{
		XMFLOAT3 pos = m_SkinnedVertices[i].OriginalVertex.Position;
		XMVECTOR vec = XMVector3Transform(XMLoadFloat3(&pos), boneTransform1);
		XMStoreFloat3(&m_SkinnedVertices[i].TransformedVertex.Position, vec);
	}
...
void SoftwareSkinningScene_2::SceneActivated()
{
}

void SoftwareSkinningScene_2::SceneDeactivated()
{
}
...
void SoftwareSkinningScene_2::CreateMesh(float length)
{
	...
	m_SkinnedVertices.push_back(SkinnedVertex(XMFLOAT3(-offset.x + pos.x, -offset.y + pos.y, -offset.z + pos.z), norm, col));
	//BACK
	norm = XMFLOAT3(0, 0, 1); m_SkinnedVertices.push_back(SkinnedVertex(XMFLOAT3(offset.x + pos.x, offset.y + pos.y, offset.z + pos.z), norm, col));
	m_SkinnedVertices.push_back(SkinnedVertex(XMFLOAT3(-offset.x + pos.x, offset.y + pos.y, offset.z + pos.z), norm, col));
	m_SkinnedVertices.push_back(SkinnedVertex(XMFLOAT3(-offset.x + pos.x, -offset.y + pos.y, offset.z + pos.z), norm, col));
	m_SkinnedVertices.push_back(SkinnedVertex(XMFLOAT3(offset.x + pos.x, -offset.y + pos.y, offset.z + pos.z), norm, col));
	//TOP
